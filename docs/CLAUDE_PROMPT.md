...
返回第一段仍是 `合并→桶名` 或 `新建→桶名`，后面会附一行「落点」证据：
最邻近旧桶、0~100 的多维邻近分、合并线，以及为什么合并/新建。这个分数不是
纯向量相似度；超过合并线后仍须通过“同一事件”判定。只有普通 dynamic 桶参与
自动合并；pinned/protected/permanent/feel/plan/letter/I 都不会被普通 hold 改写。
逐字重复会跳过正文追加。`feel=True` 会返回本地向量回声；若向量仍在后台队列，
只说明稍后由 dream 观察，不阻塞原文落盘。
